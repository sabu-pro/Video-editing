export async function makeAVFixture(page, withAudio=true, seconds=4){
  const base64=await page.evaluate(async({withAudio,seconds})=>{
    ...
    if(withAudio){for(let i=0;i<2;i++){const osc=audio.createOscillator(),gain=audio.createGain();osc.frequency.value=i?660:440;gain.gain.value=i?.12:.6;osc.connect(gain);gain.connect(merger,0,i);osc.start();oscillators.push(osc);}merger.connect(destination);stream.addTrack(destination.stream.getAudioTracks()[0]);}
